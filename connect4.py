    ...
    def get_winner(self):
        for i in range(self.NUM_ROWS - self.NUM_TO_WIN + 1):
            for j in range(self.NUM_COLS - self.NUM_TO_WIN + 1):
                ...
                sub_board = self.board[
                    i : (i + self.NUM_TO_WIN), j : (j + self.NUM_TO_WIN)
                ]
                ...
                if np.max(np.abs(np.sum(sub_board, 1))) == self.NUM_TO_WIN:
                    ...
                    np.abs(sum([sub_board[k, k] for k in range(self.NUM_TO_WIN)]))
                    == self.NUM_TO_WIN
                ):  # diag
                    return True
                elif (
                    np.abs(
                        sum(
                            [
                                sub_board[k, self.NUM_TO_WIN - 1 - k]
...
def main():
    XO = {0: "Nobody", 1: "Player X", -1: "Player O"}
    my_game = Connect4()
